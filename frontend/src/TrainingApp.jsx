...
import { setAccessToken, getAccessToken, refreshAccessToken, clearTokens } from './lib/auth'
...
export default function TrainingApp() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        if (urlToken) {
          ...
          // Strip token from browser address bar immediately to prevent history/referer leakage
        ...
        if (activeToken) {
          // Check token expiry if payload is decodable
          try {
    ...
    // Listen for storage events (e.g. logout in another tab on Port 5173)
