          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-4 bg-bg-card rounded-xl border border-bg-border p-5 shadow-card space-y-4 h-fit">
              <div className="flex items-center justify-between">
                ...
                {/* 3-Service Health Check Dots */}
                <div className="flex items-center gap-2 text-[10px] bg-bg border border-bg-border px-2.5 py-1 rounded-full">
                  <span className="flex items-center gap-1 text-emerald-400" title="Backend API (Port 8000)">
                    <span className={`w-1.5 h-1.5 rounded-full ${camerasError ? 'bg-rose-500 animate-pulse' : 'bg-emerald-400'}`} />
                    <span>8000</span>
                  </span>
                  <span className="text-text-muted">·</span>
                  <span className="flex items-center gap-1 text-cyan-400" title="Streamer (Port 8001)">
                    <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />
                    <span>8001</span>
                  </span>
                  <span className="text-text-muted">·</span>
                  <span className="flex items-center gap-1 text-purple-400" title="Training Engine (Port 8002)">
              ...
              {/* Differentiated Connection / Auth Banners */}
              {camerasError && (
                <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs space-y-2">
                  <div className="flex items-start justify-between gap-2 font-semibold">
                    <span>
                      {String(camerasError).includes('401')
                        ? '⚠️ Auth required on Port 5174'
                        : '⚠️ Backend API (8000) Unreachable'}
                    </span>
                    <button
                      onClick={() => refetchCameras()}
                      className="px-2 py-0.5 rounded bg-rose-500/20 hover:bg-rose-500/40 text-rose-200 text-[10px] border border-rose-500/40 transition-colors"
                    >
                      Retry
                    </button>
                  </div>
                  <p className="text-[11px] text-rose-300/80 leading-normal">
                    {String(camerasError).includes('401')
                      ? 'Access token missing or expired on Port 5174. Open from Dashboard to auto-sync session.'
                      : camerasError}
                  </p>
                </div>
              )}
              ...
              {!camerasLoading && !camerasError && cameras.length === 0 && (
                <div className="p-2.5 rounded-lg bg-slate-800/40 border border-slate-700/50 text-slate-300 text-xs flex items-center justify-between">
                  <span>ℹ No active cameras in database.</span>
                  <button
                    onClick={() => refetchCameras()}
                    className="text-[10px] px-2 py-0.5 rounded bg-slate-700 hover:bg-slate-600 text-slate-200 transition-colors"
                  >
                    Refresh
                  </button>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <select 
                    ...
                    className="bg-bg border border-bg-border rounded-lg px-3 py-2 text-sm text-text-primary flex-1 focus:outline-none focus:border-accent-cyan"
                  >
                    {camerasLoading && <option value="">Loading cameras...</option>}
                    {!camerasLoading && <option value="">Select Camera...</option>}
                    {cameras.map(cam => {
                  ...
                  <button
                    ...
                    className="p-2 bg-bg border border-bg-border rounded-lg text-text-secondary hover:text-accent-cyan hover:border-accent-cyan/40 transition-colors"
                  >
